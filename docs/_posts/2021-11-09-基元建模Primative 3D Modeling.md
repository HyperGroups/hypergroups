...
本文没有那么严格，只是粗略在分类和讨论和介绍，以下[基元]一词和[积木]一词等价，在偏理论一点的环境，基元是更自然的一个描述，有MeshPrimitive, a graphics primitive,primitive也会出现在许多模型语义分割的论文里。

通常来讲，我们最常见的模型类型有：多边形面片模型，四面体与六面体网格模型，体素模型，点云模型，参数曲面模型等，并且他们本身之间也存在直接和间接的转换。

按普通大众比较熟悉的是多边形模型[大部分游戏里看到的]，体素模型[我的世界]，虽然四面体、六面体是个比体素更广泛的体素，为了简化，我们这里体素专门指的就是立方体[含长方体]。

那么我这里介绍的积木建模，一是角色[物件]建模，二是场景建模。

角色建模主要聚集于构成一个模型，并不是常见的人形动物的角色的意思，主要指的就是物体模型，但是这个模型有许多积木构成。

场景建模聚集于一个场景，会有大量的模型，每个模型可能是一个积木，也可能由多个积木构成。
...
通常来讲，基元的复杂度是不高的，就是些基本图案、基础模型，比如环、方块、圆环等。

### 点线面

点线面，已经是一种3维空间维度的最低基元了。
线最基本的主要是一个直线段，然后可以拼成许多折线，也可以是样条曲线等由参数曲线。
面最基本的是一个三角面或任意封闭平面，扩展一下也可以是参数曲面等。
...
基本可变参数，积木作为一个整体，可以进行按三个轴缩放、位移、旋转、镜像。
